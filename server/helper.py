...
def make_text_summary(data_summary):
...
def make_summary(df):
    for index, row in df.iterrows():
        ...
        for document in myCollection.find():
            score = 0
            mrnSimilarityScore = lev.ratio(
                document.get('MRN').lower(), mrn_inp.lower())
            if mrnSimilarityScore >= mrnWeight:
                score = score + 1

            fnameSimilarityScore = lev.ratio(document.get(
                'First Name').lower(), fname_inp.lower())
            if fnameSimilarityScore >= fnameWeight:
                score = score + 1

            lnameSimilarityScore = lev.ratio(document.get(
                'Last Name').lower(), lname_inp.lower())
            ...
            phoneSimilarityScore = lev.ratio(
                str(document.get('Phone Number')), phone_inp)
            ...
            pincodeSimilarityScore = lev.ratio(
                str(document.get('Pincode')), pincode_inp)
            ...
            spezSimilarityScore = lev.ratio(
                document.get('Specialization'), spez_inp)
    ...
    # text_summary = make_text_summary(data_summary)
    return data_summary
