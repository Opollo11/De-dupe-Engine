...
# print("Similar Names - ")
# for name in df['name']:
#     if(lev.ratio(name_in.lower(),name.lower())>=nameThreshold):
#         print(name)
'''
print("Similar names in database - ")

for x in range(0,11214):
    if(name_list[x]>=nameThreshold):
        print(df.name[i])
'''
'''
Str1 = "Chandru R"
Str2 = "Chandru Raghavan"
...
# Now calculating the similarity score in terms of % - Here we've considered two columns only.
# So in the case of exact match also, the score would have a value of 200. If we considered 3 columns, it would be 300, 400 for 4 coulms and so on.
